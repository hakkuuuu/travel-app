  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
      ...
      <div className="bg-white rounded-xl shadow p-4 sm:p-6 border border-gray-100">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-4">Recent Destinations</h3>
        {destinations.slice(0, 5).map((dest, idx) => (
          <div key={dest.id ?? `dest-${idx}`} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
            <div>
              <p className="font-medium text-gray-900 text-sm sm:text-base">{dest.name}</p>
              <p className="text-xs sm:text-sm text-gray-600">{dest.location}</p>
            </div>
            <span className="text-xs sm:text-sm text-gray-500">{dest.price}</span>
          </div>
        ))}
      </div>
      ...
      {/* Recent Users */}
      <div className="bg-white rounded-xl shadow p-4 sm:p-6 border border-gray-100">
        <h3 className="text-base sm:text-lg font-semibold text-gray-800 mb-2 sm:mb-4">Recent Users</h3>
